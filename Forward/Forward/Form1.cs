...
namespace Forward
{
    public partial class Form1 : Form
    {
        ...
        void pusi(Excel.Worksheet currentSheet, Excel.Application excelApp)
        {
            ...
            //для чтения из ячеек ищем ебучий максимальный диапозон
            Excel.Range range;
            ...
            {//!!!!!!!!!!!!!!сделать обработчик выведет сообщение о том что вся таблица пустая!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    //получили первый маил
                    {
                        ...
                        for (int cCft = 1; cCft <= MaxCol; cCft++)
                        {
                            ...
                            //первый проход проверяет первуюячейку ниже код   
                            //надо въебать условия проверки типов на ебучую первую колонку
                            //еще надо обернуть это все в проверку нулевой ссылки
                            if ((range.Cells[cCnt, cCft] as Excel.Range).Value2 != null)//проверяет каждую ябучую ячейку на содержание внутри
                            {   //тут можно вхуярить свитч ебучий в принципе не так то сложно(тут он в

                                // если не пустая ячейка, тогда все тут крутиться, закономерно выявить какие данные получает ебана один проход
                                if ((range.Cells[cCnt, cCft] as Excel.Range).Value2.GetTypeCode() == TypeCode.String)//если попался доубля то он идет вниз
                                {
                                    ...
                                    /* doc.Bookmarks["DATAREG"].Range.Text = dat;//принято в работу 1ый столбец
                                     ...
                                     //7 столбец это номер телефона он нахер никому не упал
                                     //8 нахер не нужен это адрес для почты заяввителя
                                     //9 столбец представитель тоже нахер не нужна
                                     //10 столбец электронная почта представителя нигде не указывается
                                     //11 столбец адрес почты представителя, тоже в жопу его
                                     ...
                                     //14 номер просто номер тоже нахер пока не нужен
                                     ...
                                     //18 количество копий  тоже не важная херня
                                     //19 количество листов в копиях хера Вам надо это вообще хуйню пишут тут но в ворде не пишут
                                     doc.Bookmarks["KN"].Range.Text = dat;//20 столбец кадастровый номер
                                     doc.Bookmarks["ADRESS"].Range.Text = dat;//21 столбец там ебучий адрес он пихается в ворд(тоже надо редактировать
                                     doc.Bookmarks["PLANDOG"].Range.Text = dat;//22 столбец срок завершения тоже в ворде
                                     doc.Bookmarks["TYPEOBJ"].Range.Text = dat;//23 столбец тип объекта типа земельный участок и еще херня какая нибудь*/
        ...
        private void result_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                excelApp.Workbooks.Open(tbExcel.Text);//получили рабочую книгу можно хуярить
                currentSheet = (Excel.Worksheet)excelApp.Workbooks[1].Worksheets[1];
